...
public class App {
	 ...
	 */
		public static int speedingFine(int speed, boolean isHoliday) {
			int fine = 0;
			if(!isHoliday) {
				if(speed<60 || speed == 60 ) fine = 0;//System.out.println("fine amt"+fine);
				if(speed >=61 && speed <=80) fine = 100;
				if(speed >= 81) fine = 200;
			}else if(isHoliday){
				if(speed<=65 ) fine = 0;
				if(speed >=66 && speed <=85) fine = 100;
				if(speed >= 86) fine =200;
			}
